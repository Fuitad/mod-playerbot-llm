...
- Which mode is in force is decided by `AiPlayerbot.SocialChat.Enable`. While it is on, this module registers with the coordinator and the legacy whisper, party, and ambient hooks stand down, so one message can never produce two answers chosen by two different rules. `PlayerbotsLLM.AmbientWorldEnable` is then reported as ignored in the log rather than silently skipped.
...
- Social uses channel `social` and declares `kind: "social"`. A social request carries the coordinator's correlation token, a bot actor and an optional subject actor in one shared field shape differing only in a `human` flag, the channel the line should be spoken on, an opaque thread identity, and a bounded context. It also carries what the latest line asked and of whom, as two trusted worldserver decisions: `expects_answer` (whether that line asked a question at all, the same signal the delivery gate already judges the reply against) and `addressed_to_bot` (whether it was aimed at THIS bot rather than at somebody else in the same room). A question overheard in a room is not a question to answer in the addressee's place, and only the coordinator knows which of the two it is. The bot's own `bot_race_id`, `bot_class_id`, and `bot_zone` travel alongside so the prompt premise can say who the bot is rather than leave the model to invent it; the zone is bounded to 64 UTF-8 bytes on a single line, because it is interpolated into the trusted system prompt. A deliverable Social response echoes the correlation token and bot identity plus a complete provider metadata set: `model`, `provider_latency_ms`, `input_tokens`, `output_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`, and `cost_usd`. Model names are nonempty and at most 64 UTF-8 bytes. Time and token values are nonnegative integers. Cost is the exact settled microdollar value with six fractional digits. The latency clock starts immediately before `generate_social_reply` and stops immediately after it returns, using complete milliseconds from a monotonic clock. A response may instead set `regenerate`, which carries no message or call metadata and is honoured at most once per request.
...
- Every payload carries the bridge token from `PLAYERBOTS_LLM_BRIDGE_TOKEN`. Both sides compare it in constant time. A mismatch closes the connection without revealing the expected value. Both processes fail closed at startup when the token is missing or shorter than 32 bytes.
...
- The API key is read from `MOD_PLAYERBOTS_LLM_ANTHROPIC_API_KEY` only by the Anthropic adapter and passed to the SDK explicitly. The SDK's implicit `ANTHROPIC_API_KEY` fallback is disabled by construction, so a machine-wide key can never be used by this module.
...
3. **Settle.** After a successful reply, the reported usage replaces the maximum. Settlement returns whether the row completed, the exact stored nano amount, and separate breach and saturation flags. If the reported cost exceeds what was reserved, the circuit breaker opens, the storable figure is recorded, and every later request is denied. A Social answer is returned only when settlement completed with both flags false. An expired reservation, an overrun, or a saturated day therefore stays silent and cannot present a clamped or discarded value as the provider cost.
